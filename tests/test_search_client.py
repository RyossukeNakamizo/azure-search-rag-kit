...
class TestAzureSearchClient:
    ...
    def test_client_initialization_success(self, mock_env, mock_credential):
        ...
        with patch("search_client.DefaultAzureCredential", return_value=mock_credential):
            with patch("search_client.SearchIndexClient"):
                with patch("search_client.SearchClient"):
    ...
    def test_generate_embedding(self, mock_env, mock_credential, mock_openai_client):
        ...
        with patch("search_client.DefaultAzureCredential", return_value=mock_credential):
            with patch("search_client.SearchIndexClient"):
                with patch("search_client.SearchClient"):
                    with patch("search_client.AzureOpenAI", return_value=mock_openai_client):
                        client = AzureSearchClient()
...
class TestSearchOperations:
    ...
    def test_search_returns_results(self, mock_env, mock_credential, mock_search_client):
        ...
        with patch("search_client.DefaultAzureCredential", return_value=mock_credential):
            with patch("search_client.SearchIndexClient"):
                with patch("search_client.SearchClient", return_value=mock_search_client):
                    client = AzureSearchClient()
